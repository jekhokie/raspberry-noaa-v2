...
if [ "$RECEPTION_TYPE" == "record" ]; then
  ...
  $SOX "${RAMFS_AUDIO_BASE}-baseband.wav" -r 110250 "${RAMFS_AUDIO_BASE}-baseband-resampled.wav" >> $NOAA_LOG 2>&1
  ...
  rm satdump.log product.cbor dataset.json "${RAMFS_AUDIO_BASE}-baseband.wav" "${RAMFS_AUDIO_BASE}-baseband-resampled.wav" >> $NOAA_LOG 2>&1
