...
$FFMPEG -y -i ${IN_WAV_FILE} -lavfi showspectrumpic=s=400x800:orientation=1:saturation=1:gain=0.3:color=intensity:start=1300:stop=4000:legend=1 temporary.png

sleep 2

# add the title text
$FFMPEG -y -i temporary.png -vf "drawtext=fontfile=/path/to/font.ttf:text='${CHART_TEXT}':fontcolor=white:fontsize=18:box=1:boxcolor=black@0.5:boxborderw=5:x=20:y=10" ${OUT_PNG_FILE}
